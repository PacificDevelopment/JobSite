...
const quickApply = async (session_id, job_post_id) => {
  const user = await pool.query('SELECT user_id FROM users WHERE session_id = $1', [session_id]);
  const resume = await pool.query('SELECT id FROM resumes WHERE user_id = $1', [user]);
  const coverLetter = await pool.query('SELECT id FROM cover_letters WHERE user_id = $1', [user]);
  const employerID = await pool.query('SELECT employer_id FROM job_posts WHERE id = $1', [job_post_id]);

  const response = await pool.query(
    'INSERT into Applications (employer_id, cover_letter_id, resume_id, job_post_id, user_id) values ($1, $2, $3, $4, $5)',
